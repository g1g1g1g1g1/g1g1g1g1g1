Hi, I'm Jiwon 👋

### 💼 Experience
**Senior Associate Software Engineer** @ Capital One `[Jul. 2026 – Present]`
- Turing Team
- Developing in-house distributed cache using Rust, RocksDB, and Redis (RESP)

**Associate Software Engineer** @ Capital One `[Aug. 2025 – Jul. 2026]`
- Bank Modernization Team
- Built microservices using Go, AWS ECS Fargate, and DynamoDB

### 🌱 Interests
- Distributed Backends & Low-Latency Systems
- Competitive Programming & Algorithms
- Systems Development

### 🏫 Education
**Bachelor of Science in Computer Science** @ Purdue University `[Aug. 2021 – May 2025]`
